...
local function scan_conflicts(bufnr)
...
function M.detect_conflicts(bufnr, silent)
  ...
  if #conflicts > 0 then
    ...
    if config.on_conflict_detected then
      ...
      if not ok then
        vim.notify_once("Error in on_conflict_detected hook: " .. tostring(err), vim.log.levels.ERROR)
      end
    end
  else
...
function M.highlight_conflicts(conflicts)
  ...
  for _, conflict in ipairs(conflicts) do
    ...
    vim.api.nvim_buf_set_extmark(bufnr, ns_id, conflict.ours_start - 1, 0, {
      ...
      virt_text = { { config.virtual_text.ours, "ResolveOursMarker" } },
    ...
    if conflict.ancestor then
      vim.api.nvim_buf_set_extmark(bufnr, ns_id, conflict.ancestor - 1, 0, {
        ...
        virt_text = { { config.virtual_text.ancestor, "ResolveAncestorMarker" } },
        virt_text_pos = 'overlay',
    ...
    vim.api.nvim_buf_set_extmark(bufnr, ns_id, conflict.theirs_end - 1, 0, {
      ...
      virt_text = { { config.virtual_text.theirs, "ResolveTheirsMarker" } },
      virt_text_pos = 'overlay',
...
function M.next_conflict()
  ...
  vim.notify_once("No more conflicts", vim.log.levels.INFO)
end
...
function M.prev_conflict()
  ...
  vim.notify_once("No previous conflicts", vim.log.levels.INFO)
end
...
function M.choose_ours()
  ...
  if not conflict then
    vim.notify_once("Not in a conflict", vim.log.levels.WARN)
    return
  end
...
function M.choose_base()
  ...
  if not conflict.ancestor then
    vim.notify_once("No base version available (not a diff3-style conflict)", vim.log.levels.WARN)
    return
  end
...
function M.list_conflicts()
  ...
  if #conflicts == 0 then
    vim.notify_once("No conflicts found", vim.log.levels.INFO)
    return
  end
...
local function show_diff_internal(show_base_ours, show_base_theirs, show_ours_theirs, show_theirs_ours)
  ...
  if (show_base_ours or show_base_theirs) and not conflict.ancestor then
    vim.notify_once("Not a diff3-style conflict (no base version)", vim.log.levels.WARN)
    return
  end
